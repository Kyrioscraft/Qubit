    ...
    <Card variant="interactive" padding="md">
      <Link href={`/garden/${node.slug}`}>
        ...
        <div className="flex items-center justify-between gap-3 pt-3 border-t border-[var(--color-border-subtle)]">
          <TagList tags={node.tags.slice(0, 3)} size="sm" variant="muted" />
          <div className="flex items-center gap-2 text-xs text-[var(--color-text-subtle)] shrink-0">
            ...
            <span className="flex items-center gap-1 opacity-60">
              ...
              {node.linksTo.length}/{node.linkedFrom.length}
            </span>
          </div>
        </div>
      </Link>
